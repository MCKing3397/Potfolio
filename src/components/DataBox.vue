  ...
  <div class="databox tile is-ancestor">
    <div class="tile is-parent columns is-centered"> 
      ...
      <article class="tile is-child is-4 box is-radiusless">
        ...
        <p class="content" v-else><ul><li v-for="item in skills" v-bind:key="item.id"><progress class="progress is-white" v-bind:value="item.level * 10" max="100"></progress> {{ item.level }}</li></ul></p>
      </article>
    </div>
  </div>
</template>
